...
On fait une repréentation graphique de l'arbre du jeu :
...
[
  ...
  [
    {$A$ : accepte / refuse}
    [
      {Refuse\\ \textbf{Gains :} $(0,\,u_r)$}
    ]
    [
      {Accepte}
      [
        {$A$ : choisit $e\in\{H,B\}$\\ \textit{(non observé par $P$)}}
        [
          {$e=H$}
          [
            {Nature}
            [
              {Succès\\ prob. $\pi_H$\\ \textbf{Gains :} $(\bar q-\bar w,\; u(\bar w)-d_H)$}
            ]
            [
              {Échec\\ prob. $1-\pi_H$\\ \textbf{Gains :} $(q-w,\; u(w)-d_H)$}
            ]
          ]
        ]
        [
          ...
          [
            {Nature}
            [
              {Succès\\ prob. $\pi_B$\\ \textbf{Gains :} $(\bar q-\bar w,\; u(\bar w)-d_B)$}
            ]
            [
              {Échec\\ prob. $1-\pi_B$\\ \textbf{Gains :} $(q-w,\; u(w)-d_B)$}
            ]
          ]
        ]
      ]
...
digraph PA {
  ...
  P  [label='P : propose (w̄, w)'];
  A1 [label='A : accepte / refuse'];
  R  [label='Refuse\\nGains : (0, u_r)'];

  A2 [label='A : choisit e ∈ {H,B}\\n(non observé par P)'];

  NH [shape=ellipse, label='Nature | e=H'];
  NB [shape=ellipse, label='Nature | e=B'];

  SH [label='Succès (π_H)\\n( q̄ - w̄ , u(w̄) - d_H )'];
  FH [label='Échec (1-π_H)\\n( q - w , u(w) - d_H )'];

  SB [label='Succès (π_B)\\n( q̄ - w̄ , u(w̄) - d_B )'];
  FB [label='Échec (1-π_B)\\n( q - w , u(w) - d_B )'];
